...
-- PERFORMANCE INDEXES
-- =========================
-- Índices para otimizar consultas (dashboard / listagens)
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE table_name = 'estoque' AND column_name = 'ativo'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'estoque' AND column_name = 'tag_patrimonio'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.tables WHERE table_name = 'execucao'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.tables WHERE table_name = 'oficina'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.tables WHERE table_name = 'manutencao'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.tables WHERE table_name = 'notificacao'
    ) THEN
